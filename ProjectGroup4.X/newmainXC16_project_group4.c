...
int main(void) {
    ...
    cd.par_state = ps; // used to read speed and yaw
    cd.obs_av_state_ctrl = 0; // variable to ensure obstacle avoidance mechanic is execute only 3 times
    cd.angle_values[0] = 0;
    cd.angle_values[1] = 0;
    cd.angle_values[2] = 0;
    cd.one_time_exec = 0; // flag to updated variables only one time in task_PWM_set
