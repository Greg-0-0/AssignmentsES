...
void task_stop_buggy_after_2sec (void* param){
    // aperiodic task that is enabled only when the robot is moving forward after rotating 90° clockwise (AVOIDANCE_STEP_2)
...
void task_reading_IR_value(void* param){
    ...
    if(cd->robot_state != HALTED_STATE){
        if(distance <= 35 && cd->robot_state == MOVING_STATE && cd->obs_av_state_ctrl < 3){
            ...
            // obstacle closer than 35 centimetres after obstacle avoidance manoeuvre (after rotating of 90° anti-clockwise)
            ...
            // during moving forward after rotating of 90° clockwise, the robot is too close to the obstacle -> 
...
void task_reading_magn_acc_gyro(void* param){
    ...
    if(cd->robot_sub_state == AVOIDANCE_STEP_1){
        ...
        float diff = cd->gyro_yaw - cd->ctrl_yaw;
        // to ensure that the difference is always in the range [-180;180] (to avoid problems with 0°/360° transition)
        while(diff > 180.0f) diff -= 360.0f;
        while(diff < -180.0f) diff += 360.0f;
